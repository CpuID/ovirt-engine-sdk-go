...
public class ServicesGenerator implements GoGenerator {
    ...
    private void generatePathLocator(Service service) {
        ...
        service.locators().filter(x -> x.getParameters().isEmpty()).sorted().forEach(locator -> {
            ...
            buffer.addLine(  "return %1$s_service", goNames.getPublicFuncStyleName(name));
            buffer.addLine("end");
            buffer.addLine("if path.start_with?('%1$s/')", segment);
            buffer.addLine(
                "return %1$s_service.service(path[%2$d..-1])",
                goNames.getPublicFuncStyleName(name),
                segment.length() + 1
            );
            buffer.addLine("end");
        });
        ...
        if (optional.isPresent()) {
            ...
            buffer.addLine(  "return %1$s_service(path)", goNames.getPublicFuncStyleName(name));
            buffer.addLine("end");
            buffer.addLine(
